...
public class DRController extends Controller{
	static DiagnosticRepresentative loggedInDR = LoginController.getLoggedInUser().getDiagnosticRepresentative();
	public static Form<DiagnosticRepresentative> diagnosticRepresentative=Form.form(DiagnosticRepresentative.class);
	
	
	public static Result addDiagRep(){
		return ok(views.html.diagnostic.diagnosticRep.render(diagnosticRepresentative));
	}
	public static Result addDiagRepProcess(){
		final Form<DiagnosticRepresentative> filledForm=diagnosticRepresentative.bindFromRequest();
	...
	public static Result diagnosticReplist() {
		List<DiagnosticRepresentative> allDiagRepList=DiagnosticRepresentative.find.all();
	...
	public static Result addDoctor( Long id){
		Logger.info("id.............."+id);
		DiagnosticRepresentative loggedDr = LoginController.getLoggedInUser().getDiagnosticRepresentative();
		if(loggedDr.doctorList.contains(Doctor.find.byId(id))!=true){
			loggedDr.doctorList.add(Doctor.find.byId(id));	
		}
	...
	public static Result search(){
		...
		if (searchStr != null && !searchStr.isEmpty()) {
			...
			if (searchStr.matches("[a-zA-Z]+")) {
				...
				return redirect(routes.DRController.doctorList());
			}
